...
from Truss_stem_solved import TrussController
import sys
...
class MainWindow(Ui_TrussStructuralDesign,qtw.QWidget):
    ...
    def OpenFile(self):
        # Open a file dialog to let the user select a file, and obtain the file path
        filename = qtw.QFileDialog.getOpenFileName()[0]
        if filename:  # Check if a file was selected
            self.te_Path.setText(filename)  # Display the selected file path in the UI
            self.controller.ImportFromFile(filename)  # Pass the file path to the controller
        else:
